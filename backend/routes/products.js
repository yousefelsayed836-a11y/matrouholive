...
router.post('/', async (req, res) => {
  try {
    const { name_en, name_ar, description_en, description_ar, price, old_price, images, main_image, category_id, category_ids, material, water_resistance, size_info, stock, is_active, is_featured, matruh_only, variants } = req.body;
    ...
    await runQuery(`INSERT INTO products (id, name_en, name_ar, description_en, description_ar, price, old_price, images, main_image, category_id, material, water_resistance, size_info, stock, is_active, is_featured, matruh_only) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [id, name_en, name_ar || name_en, description_en || null, description_ar || null, price, old_price || null, JSON.stringify(imagesArr), mainImg, resolvedCategoryId, material || null, water_resistance || null, size_info || null, stock || 0, is_active !== undefined ? (is_active ? 1 : 0) : 1, is_featured ? 1 : 0, matruh_only ? true : false]);
